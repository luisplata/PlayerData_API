exports.up = function (knex) {
  return Promise.all([
    knex.schema.alterTable('heroes', table => {
      table.index('heroId', 'idx_heroes_heroId');
    }),
    knex.schema.alterTable('passives', table => {
      table.index('heroId', 'idx_passives_heroId');
    }),
    knex.schema.alterTable('dialogs', table => {
      table.index('heroId', 'idx_dialogs_heroId');
    }),
    knex.schema.alterTable('dialog_questions', table => {
      table.index('dialogId', 'idx_dialog_questions_dialogId');
    }),
    knex.schema.alterTable('player_passives', table => {
