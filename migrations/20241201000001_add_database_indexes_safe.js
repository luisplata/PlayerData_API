...
exports.up = async function (knex) {
  ...
  const createIndexIfNotExists = async (tableName, indexName, columns) => {
    ...
    if (!exists) {
      try {
        ...
        console.log(
          `Index ${indexName} already exists or error: ${error.message}`
        );
      }
    } else {
  ...
  await createIndexIfNotExists(
    'battle_passes',
    'idx_battle_passes_playerId',
    'playerId'
  );
  await createIndexIfNotExists(
    'battle_passes',
    'idx_battle_passes_level',
    'level'
  );
  await createIndexIfNotExists(
    'battle_passes',
    'idx_battle_passes_experience',
    'experience'
  );
  ...
  await createIndexIfNotExists(
    'battle_pass_rewards',
    'idx_battle_pass_rewards_level',
    'level'
  );
  await createIndexIfNotExists(
    'battle_pass_rewards',
    'idx_battle_pass_rewards_created_at',
    'created_at'
  );
  ...
  await createIndexIfNotExists(
    'player_rewards',
    'idx_player_rewards_playerId',
    'playerId'
  );
  await createIndexIfNotExists(
    'player_rewards',
    'idx_player_rewards_rewardId',
    'rewardId'
  );
  await createIndexIfNotExists(
    'player_rewards',
    'idx_player_rewards_claimed',
    'claimed'
  );
  await createIndexIfNotExists(
    'player_rewards',
    'idx_player_rewards_awarded_at',
    'awarded_at'
  );
  await createIndexIfNotExists(
    'player_rewards',
    'idx_player_rewards_player_reward',
    'playerId, rewardId'
  );
};
