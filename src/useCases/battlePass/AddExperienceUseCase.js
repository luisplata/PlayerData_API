    ...
    try {
      ...
      const result = await this.transactionService.executeTransaction(
        async trx => {
          ...
          await this.battlePassRepository.update(
            playerId,
          ...
          if (experienceResult.levelUp) {
            ...
            for (let i = 1; i <= levelsGained; i++) {
              ...
              const reward =
                await this.battlePassRewardRepository.findByLevel(newLevel);
              if (reward) {
                // Check if player already has this reward
                const existingReward =
                  await this.playerRewardRepository.findByPlayerIdAndLevel(
                    playerId,
                    newLevel
                  );
                if (!existingReward) {
                  await this.playerRewardRepository.awardReward(
                    playerId,
                    newLevel
                  );
                }
          ...
          let currentReward = await this.battlePassRewardRepository.findByLevel(
            experienceResult.level
          );
          ...
          if (!currentReward) {
            ...
            if (allRewards && allRewards.length > 0) {
              const sortedRewards = allRewards.sort(
                (a, b) => b.level - a.level
              );
