...
describe('API Versioning Tests', () => {
  let authToken;
  const testPlayerId = `version_test_${Date.now()}`;
  const testNickname = `versionuser_${Date.now()}`;
    ...
    it('should include v1 version info', done => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body.data.versionInfo.v1).to.have.property(
            'version',
            '1.0.0'
          );
          expect(res.body.data.versionInfo.v1).to.have.property(
    ...
    it('should include inactive version info for legacy and v2', done => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body.data.versionInfo.v2).to.have.property(
            'status',
            'inactive'
          );
          expect(res.body.data.versionInfo).to.have.property('legacy');
          expect(res.body.data.versionInfo.legacy).to.have.property(
            'status',
            'inactive'
          );
          expect(res.body.data.versionInfo.legacy).to.have.property(
