...
describe('Heroes migration integrity (Phase 5.2)', () => {
  it('enforces FK and unique constraints for player_passives', async () => {
    ...
    expect(
      createCall.constraints.some(
        c => c.type === 'unique' && c.column === 'playerId,heroId,passiveId'
      )
    ).to.equal(true);
  });
  ...
  it('supports rollback paths for heroes schema tables (down drops created tables)', async () => {
    const migrationFiles = [
      {
        file: '../../migrations/20260417000100_create_heroes_table',
        table: 'heroes'
      },
      {
        file: '../../migrations/20260417000101_create_passives_table',
        table: 'passives'
      },
      {
        file: '../../migrations/20260417000102_create_dialogs_table',
        table: 'dialogs'
      },
      {
        file: '../../migrations/20260417000103_create_dialog_questions_table',
        table: 'dialog_questions'
      },
      {
        file: '../../migrations/20260417000104_create_player_passives_table',
    ...
    for (const item of migrationFiles) {
      ...
      expect(
        knex.calls.some(c => c.type === 'dropTable' && c.name === item.table)
      ).to.equal(true);
    }
  });
